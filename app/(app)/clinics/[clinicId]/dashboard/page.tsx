...
export default function ClinicDashboardPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      description: "Registrados en esta clínica",
      icon: Users,
      color: "text-teal-600",
      bgColor: "bg-gradient-to-br from-teal-50 to-teal-100",
      borderColor: "border-teal-200",
      tooltip: "Número total de pacientes en esta clínica",
    },
    {
      title: "Pendientes",
                    ...
                    >
                      ...
                      <div className="text-right shrink-0">
                        <p className="font-bold text-foreground text-lg">
                          {appointment.hora || new Date(appointment.appointment_datetime).toLocaleTimeString("es-DO", { hour: "2-digit", minute: "2-digit" })}
                        </p>
                        <Badge
                          variant="secondary"
                ...
                <div className="space-y-3">
                  {activeConversations.length === 0 ? (
                    ...
                    activeConversations.slice(0, 3).map((conversation) => {
                      ...
                      const patientName = (conversation.paciente as any)?.full_name || `${(conversation.paciente as any)?.nombre || ""} ${(conversation.paciente as any)?.apellido || ""}`.trim()
                      const initials = patientName ? patientName.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase() : "??"
